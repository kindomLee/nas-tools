    ...
    def filter_search_results(self, result_array: list,
        ...
        for item in result_array:
            ...
            if meta_info.type == MediaType.TV and filter_args.get("type") == MediaType.MOVIE:
                log.info(
                    f"【{self.index_type}】{torrent_name} 是 {meta_info.type.value}，不匹配型別：{filter_args.get('type').value}")
                index_rule_fail += 1
                continue
            # 檢查訂閱過濾規則匹配
            ...
            if not match_media:
                ...
                if meta_info.imdb_id \
                    ...
                    if match_media and str(cache_info.get("id")) == str(match_media.tmdb_id):
                        ...
                        if not media_info:
                            log.warn(f"【{self.index_type}】{torrent_name} 識別媒體資訊出錯！")
                            index_error += 1
                            continue
                        elif not media_info.tmdb_info:
                            log.info(f"【{self.index_type}】{torrent_name} 識別為 {media_info.get_name()} 未匹配到媒體資訊")
                            index_match_fail += 1
                            continue
                        # TMDBID是否匹配
                        if str(media_info.tmdb_id) != str(match_media.tmdb_id):
                            log.info(
                                f"【{self.index_type}】{torrent_name} 識別為 {media_info.type.value} {media_info.get_title_string()} 不匹配")
                            index_match_fail += 1
                            continue
                        # 合併媒體資料
            ...
            if not self.filter.is_torrent_match_sey(media_info,
                ...
                log.info(
                    f"【{self.index_type}】{torrent_name} 識別為 {media_info.type.value} {media_info.get_title_string()} {media_info.get_season_episode_string()} 不匹配季/集/年份")
                index_match_fail += 1
                continue

            # 匹配到了
            log.info(
                f"【{self.index_type}】{torrent_name} {description} 識別為 {media_info.get_title_string()} {media_info.get_season_episode_string()} 匹配成功")
        ...
        log.info(
            f"【{self.index_type}】{indexer.name} 共檢索到 {len(result_array)} 條資料，過濾 {index_rule_fail}，不匹配 {index_match_fail}，錯誤 {index_error}，有效 {index_sucess}，耗時 {(end_time - start_time).seconds} 秒")
        self.progress.update(ptype='search',
                             text=f"{indexer.name} 共檢索到 {len(result_array)} 條資料，過濾 {index_rule_fail}，不匹配 {index_match_fail}，錯誤 {index_error}，有效 {index_sucess}，耗時 {(end_time - start_time).seconds} 秒")
        return ret_array
